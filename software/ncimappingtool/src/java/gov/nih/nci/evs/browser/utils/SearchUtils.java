...
public class SearchUtils {
    ...
    private void initializeSortParameters() {
        ...
        try {
            NCImtBrowserProperties properties =
                NCImtBrowserProperties.getInstance();
            String sort_str =
                properties.getProperty(NCImtBrowserProperties.SORT_BY_SCORE);
...
*/
    ...
    public ResolvedConceptReferencesIterator searchByName(
        ...
        try {
            ...
            for (int i = 0; i < matchText_vec.size(); i++) {
                ...
                if (cns != null) {
					try {
						cns = cns.restrictToMatchingDesignations(matchText, SearchDesignationOption.ALL, matchAlgorithm, language);
						//cns = restrictToSource(cns, source);
					} catch (Exception ex) {
    ...
    public ResolvedConceptReferencesIterator searchByName(
        CodedNodeSet vsd_cns, Vector matchText_vec,
        String matchAlgorithm) {
        try {
            LexBIGService lbSvc = RemoteServerUtil.createLexBIGService();
            Vector<CodedNodeSet> cns_vec = new Vector<CodedNodeSet>();
...
int j = i+1;
    ...
    public String convertPropertyValue(String s, String right_trim, String left_trim, String prefix, String suffix) {
		...
		if (left_trim != null) {
			if (left_trim.length() >= t.length()) return null;
			String substr = t.substring(0, left_trim.length());
			if (substr.compareTo(left_trim) == 0) {
				t = t.substring(left_trim.length());
			}
	    }
		...
		if (right_trim != null) {
			...
			if (right_trim.length() >= t.length()) return null;
			String substr = t.substring(t.length()-right_trim.length());
			if (substr.compareTo(right_trim) == 0) {
				t = t.substring(0, t.length()-right_trim.length());
			}
	    }
